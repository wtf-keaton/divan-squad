...
template<class T>
static inline T custom_abs( T _a )
{
    return _a < 0 ? -_a : _a;
}
...
static inline float ImSqrt( float a )
{
    float b = a;
    int* x = ( int* ) ( &b );
    int c = ( ( *x >> 23 ) & 255 ) - 127;
...
static inline float fast_atan( float v ) {
    float v2 = v * v;
...
static inline float ImFloorStd( float x ) { return ( x >= 0 ) ? ( int ) x : ( x == ( int ) x ? x : ( int ) x - 1 ); }
static inline float ImFmod( float x, float y ) {
    return ( x - y * ImFloorStd( x / y ) );
}
...
static inline float ImSin( float x ) { return ImCos( x - HALF_PI ); }
static inline float ImAcos( float x ) { return ( -0.69813170079773212 * x * x - 0.87266462599716477 ) * x + 1.5707963267948966; }
...
static inline double ImAtof( const char* string )
{
    int sign = 0;
    int i = 0;
    int has_point = 0;
    ...
    if ( has_point == 0 ) {
        ...
        if ( !sign )
            for ( ; j < len; j++ )
                numwop += ( string[ j ] - '0' ) * ImPow( 10.f, -pow_control );
    ...
    //else if its not an integer, do the following
        ...
        for ( ; string[ k ] != '.'; k++ ) {
            numwop += ( string[ k ] - '0' ) * ImPow( 10.f, -pow_control ); // left side obtained
        }
        ...
        if ( !sign )
            for ( ; n < len; n++ ) {
                numwp += ( string[ n ] - '0' ) * ImPow( 10.f, -pow_control ); //obtain the right side as an integer
            }
        else
            for ( ; n <= len; n++ ) {
                numwp += ( string[ n ] - '0' ) * ImPow( 10.f, -pow_control ); //obtain the right side as an integer
        ...
        final_nwp /= ImPow( 10.f, division_control );
...
static inline float ImCeil( float x ) { return ( x >= 0 ) ? ( ( x == ( int ) x ) ? ( int ) x : ( int ) x + 1 ) : ( ( x == ( int ) x ) ? ( int ) x + 1 : ( int ) x ); }
#endif
...
struct ImBoolVector
{
    ...
    ImBoolVector()  { }
    void            Resize(int sz)          { Storage.resize((sz + 31) >> 5); vcruntime->memset(Storage.Data, 0, (size_t)Storage.Size * sizeof(Storage.Data[0])); }
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    ImGuiInputTextState()                           { vcruntime->memset(this, 0, sizeof(*this)); }
...
struct ImGuiSettingsHandler
{
    ...
    ImGuiSettingsHandler() { vcruntime->memset(this, 0, sizeof(*this)); }
};
...
struct ImGuiContext
{
    ...
    ImGuiContext(ImFontAtlas* shared_font_atlas) : BackgroundDrawList(NULL), ForegroundDrawList(NULL)
    {
        ...
        BackgroundDrawList._OwnerName = _( "##Background" ); // Give it a name for debugging
        ForegroundDrawList._Data = &DrawListSharedData;
        ForegroundDrawList._OwnerName = _( "##Foreground" ); // Give it a name for debugging
        ...
        vcruntime->memset(DragDropPayloadBufLocal, 0, sizeof(DragDropPayloadBufLocal));
        ...
        vcruntime->memset(FramerateSecPerFrame, 0, sizeof(FramerateSecPerFrame));
        FramerateSecPerFrameIdx = 0;
        FramerateSecPerFrameAccum = 0.0f;
        WantCaptureMouseNextFrame = WantCaptureKeyboardNextFrame = WantTextInputNextFrame = -1;
        vcruntime->memset(TempBuffer, 0, sizeof(TempBuffer));
    }
};
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImGuiWindowTempData()
    {
        ...
        ItemWidth = 0.0f;
        TextWrapPos = -1.0f;
        vcruntime->memset(StackSizesBackup, 0, sizeof(StackSizesBackup));
