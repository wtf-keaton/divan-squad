...
void ImGui::StyleColorsDark( ImGuiStyle* dst )
{
    ...
    colors[ ImGuiCol_WindowBg ] = ImVec4( 33 / 255.f, 33 / 255.f, 33 / 255.f, 1.f );
    colors[ ImGuiCol_ChildBg ] = ImVec4( 20 / 255.f, 20 / 255.f, 20 / 255.f, 1.f );
    colors[ ImGuiCol_PopupBg ] = ImVec4( 50 / 255.f, 50 / 255.f, 50 / 255.f, 1.f );
    colors[ ImGuiCol_Border ] = ImVec4( 0.43f, 0.43f, 0.50f, 0.0f );
    colors[ ImGuiCol_BorderShadow ] = ImVec4( 0.00f, 0.00f, 0.00f, 0.00f );
    colors[ ImGuiCol_FrameBg ] = ImVec4( 161 / 255.f, 107 / 255.f, 255 / 255.f, 0.5f );
    colors[ ImGuiCol_FrameBgHovered ] = ImVec4( 161 / 255.f, 107 / 255.f, 255 / 255.f, 0.8f );
    colors[ ImGuiCol_FrameBgActive ] = ImVec4( 161 / 255.f, 107 / 255.f, 255 / 255.f, 1.f );
...
void ImDrawList::ClearFreeMemory( )
{
    ...
    {
        if ( i == 0 ) vcruntime->memset( &_Channels[ 0 ], 0, sizeof( _Channels[ 0 ] ) );  // channel 0 is a copy of CmdBuffer/IdxBuffer, don't destruct again
...
void ImDrawList::UpdateClipRect( )
{
    ...
    if ( !curr_cmd || ( curr_cmd->ElemCount != 0 && vcruntime->memcmp( &curr_cmd->ClipRect, &curr_clip_rect, sizeof( ImVec4 ) ) != 0 ) || curr_cmd->UserCallback != NULL )
    {
        AddDrawCmd( );
        return;
    }
    ...
    if ( curr_cmd->ElemCount == 0 && prev_cmd && vcruntime->memcmp( &prev_cmd->ClipRect, &curr_clip_rect, sizeof( ImVec4 ) ) == 0 && prev_cmd->TextureId == GetCurrentTextureId( ) && prev_cmd->UserCallback == NULL )
        CmdBuffer.pop_back( );
    else
...
void ImDrawList::UpdateTextureID( )
{
    ...
    // Try to merge with previous command if it matches, else use current command
    ImDrawCmd* prev_cmd = CmdBuffer.Size > 1 ? curr_cmd - 1 : NULL;
    if ( curr_cmd->ElemCount == 0 && prev_cmd && prev_cmd->TextureId == curr_texture_id && vcruntime->memcmp( &prev_cmd->ClipRect, &GetCurrentClipRect( ), sizeof( ImVec4 ) ) == 0 && prev_cmd->UserCallback == NULL )
        CmdBuffer.pop_back( );
    else
...
void ImDrawList::ChannelsSplit( int channels_count )
{
    ...
    vcruntime->memset( &_Channels[ 0 ], 0, sizeof( ImDrawChannel ) );
...
void ImDrawList::ChannelsMerge( )
{
    ...
    {
        ImDrawChannel& ch = _Channels[ i ];
        if ( int sz = ch.CmdBuffer.Size ) { vcruntime->memcpy( cmd_write, ch.CmdBuffer.Data, sz * sizeof( ImDrawCmd ) ); cmd_write += sz; }
        if ( int sz = ch.IdxBuffer.Size ) { vcruntime->memcpy( _IdxWritePtr, ch.IdxBuffer.Data, sz * sizeof( ImDrawIdx ) ); _IdxWritePtr += sz; }
    }
...
void ImDrawList::ChannelsSetCurrent( int idx )
{
    ...
    vcruntime->memcpy( &_Channels.Data[ _ChannelsCurrent ].CmdBuffer, &CmdBuffer, sizeof( CmdBuffer ) ); // copy 12 bytes, four times
    vcruntime->memcpy( &_Channels.Data[ _ChannelsCurrent ].IdxBuffer, &IdxBuffer, sizeof( IdxBuffer ) );
    _ChannelsCurrent = idx;
    vcruntime->memcpy( &CmdBuffer, &_Channels.Data[ _ChannelsCurrent ].CmdBuffer, sizeof( CmdBuffer ) );
    vcruntime->memcpy( &IdxBuffer, &_Channels.Data[ _ChannelsCurrent ].IdxBuffer, sizeof( IdxBuffer ) );
...
void ImDrawList::AddText( const ImFont* font, float font_size, const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end, float wrap_width, const ImVec4* cpu_fine_clip_rect )
{
    ...
    if ( text_end == NULL )
        text_end = text_begin + vcruntime->strlen( text_begin );
...
ImFontConfig::ImFontConfig( )
{
    ...
    vcruntime->memset( Name, 0, sizeof( Name ) );
    DstFont = NULL;
}
...
ImFontAtlas::~ImFontAtlas( )
{
    IM_ASSERT( !Locked && _( "Cannot modify a locked ImFontAtlas between NewFrame() and EndFrame/Render()!" ) );
    Clear( );
}
...
ImFont* ImFontAtlas::AddFont( const ImFontConfig* font_cfg )
{
    ...
    if ( !font_cfg->MergeMode )
        ...
        IM_ASSERT( !Fonts.empty( ) && _( "Cannot use MergeMode for the first font" ) ); // When using MergeMode make sure that a font has already been added before. You can use ImGui::GetIO().Fonts->AddFontDefault() to add the default imgui font.
    ...
    if ( !new_font_cfg.FontDataOwnedByAtlas )
    {
        ...
        vcruntime->memcpy( new_font_cfg.FontData, font_cfg->FontData, ( size_t ) new_font_cfg.FontDataSize );
    }
...
ImFont* ImFontAtlas::AddFontDefault( const ImFontConfig* font_cfg_template )
{
    ...
    if ( font_cfg.Name[ 0 ] == '\0' )
        ImFormatString( font_cfg.Name, IM_ARRAYSIZE( font_cfg.Name ), _( "ProggyClean.ttf, %dpx" ), ( int ) font_cfg.SizePixels );
...
ImFont* ImFontAtlas::AddFontFromFileTTF( const char* filename, float size_pixels, const ImFontConfig* font_cfg_template, const ImWchar* glyph_ranges )
{
    ...
    //    const char* p;
    //    for (p = filename + vcruntime->strlen(filename); p > filename && p[-1] != '/' && p[-1] != '\\'; p--) {}
...
{
    int compressed_ttf_size = ( ( ( int ) vcruntime->strlen( compressed_ttf_data_base85 ) + 4 ) / 5 ) * 4;
...
bool    ImFontAtlasBuildWithStbTruetype( ImFontAtlas* atlas )
{
    ...
    vcruntime->memset( src_tmp_array.Data, 0, ( size_t ) src_tmp_array.size_in_bytes( ) );
    vcruntime->memset( dst_tmp_array.Data, 0, ( size_t ) dst_tmp_array.size_in_bytes( ) );
    ...
    for ( int src_i = 0; src_i < atlas->ConfigData.Size; src_i++ )
    {
        ...
        IM_ASSERT( font_offset >= 0 && _( "FontData is incorrect, or FontNo cannot be found." ) );
    ...
    vcruntime->memset( buf_rects.Data, 0, ( size_t ) buf_rects.size_in_bytes( ) );
    vcruntime->memset( buf_packedchars.Data, 0, ( size_t ) buf_packedchars.size_in_bytes( ) );
    ...
    vcruntime->memset( atlas->TexPixelsAlpha8, 0, atlas->TexWidth * atlas->TexHeight );
...
void ImFontAtlasBuildPackCustomRects( ImFontAtlas* atlas, void* stbrp_context_opaque )
{
    ...
    vcruntime->memset( pack_rects.Data, 0, ( size_t ) pack_rects.size_in_bytes( ) );
...
const ImWchar* ImFontAtlas::GetGlyphRangesChineseSimplifiedCommon( )
{
    ...
    if ( !full_ranges[ 0 ] )
    {
        vcruntime->memcpy( full_ranges, base_ranges, sizeof( base_ranges ) );
...
{
    if ( !text_end )
        text_end = text_begin + vcruntime->strlen( text_begin ); // FIXME-OPT: Need to avoid this.
...
{
    if ( !text_end )
        text_end = text_begin + vcruntime->strlen( text_begin ); // ImGui functions generally already provides a valid text_end, so this is merely to handle direct calls.
    ...
    if ( y + line_height < clip_rect.y && !word_wrap_enabled )
        ...
        {
            s = ( const char* ) vcruntime->memchr( s, '\n', text_end - s );
            s = s ? s + 1 : text_end;
            y += line_height;
        }
    ...
    if ( text_end - s > 10000 && !word_wrap_enabled )
    {
        ...
        {
            s_end = ( const char* ) vcruntime->memchr( s_end, '\n', text_end - s_end );
...
static void stb__lit( const unsigned char* data, unsigned int length )
{
    ...
    vcruntime->memcpy( stb__dout, data, length );
    stb__dout += length;
}
