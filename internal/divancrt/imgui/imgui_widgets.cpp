...
#if __GNUC__ >= 8
#pragma GCC diagnostic ignored "-Wclass-memaccess"          // warning: 'vcruntime->memset/vcruntime->memcpy' clearing/writing an object of type 'xxxx' with no trivial copy-assignment; use assignment or value-initialization instead
#endif
#endif
...
void ImGui::TextEx( const char* text, const char* text_end, ImGuiTextFlags flags )
{
    ...
    if ( text_end == NULL )
        text_end = text + vcruntime->strlen( text ); // FIXME-OPT
    ...
    if ( text_end - text > 2000 && !wrap_enabled )
    {
        ...
        // - We use vcruntime->memchr(), pay attention that well optimized versions of those str/mem functions are much faster than a casually written loop.
        ...
        if ( !g.LogEnabled )
        {
            ...
            if ( lines_skippable > 0 )
            {
                ...
                {
                    const char* line_end = ( const char* ) vcruntime->memchr( line, '\n', text_end - line );
                    if ( !line_end )
                        line_end = text_end;
...
{
    return window->GetIDNoKeepAlive( axis == ImGuiAxis_X ? _( "#SCROLLX" ) : _( "#SCROLLY" ) );
}
...
bool ImGui::ImageButton( ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col )
{
    ...
    const ImGuiID id = window->GetID( _( "#image" ) );
    PopID();
...
bool ImGui::Checkbox( const char* label, bool* v )
{
    ...
    RenderFrame( check_bb.Min + ImVec2( 0, 2 ), check_bb.Max - ImVec2( 4, 2 ), IM_COL32( 33, 33, 33, 255 ), true, 3.f );

    if ( *v )
        window->DrawList->AddRectFilled( check_bb.Min + ImVec2( 4, 6 ), check_bb.Max - ImVec2( 8, 6 ), IM_COL32( 161, 107, 255, 255 ), 3.f );
    if ( label_size.x > 0.0f )
...
void ImGui::ProgressBar( float fraction, const ImVec2& size_arg, const char* overlay )
{
    ...
    if ( !overlay )
    {
        ImFormatString( overlay_buf, IM_ARRAYSIZE( overlay_buf ), _( "%.0f%%" ), fraction * 100 + 0.01f );
        overlay = overlay_buf;
    }
...
bool ImGui::BeginCombo( const char* label, const char* preview_value, ImGuiComboFlags flags )
{
    ...
    char name[ 16 ];
    ImFormatString( name, IM_ARRAYSIZE( name ), _( "##Combo_%02d" ), g.BeginPopupStack.Size ); // Recycle windows based on depth
...
static ImGuiDataTypeInfo GDataTypeInfo[ ] =
{
...
static const char* PatchFormatStringFloatToInt( const char* fmt )
{
    ...
    if ( fmt_end > fmt_start && fmt_end[ -1 ] == 'f' )
    {
        ...
        if ( fmt_start == fmt && fmt_end[ 0 ] == 0 )
            return _( "%d" );
        ImGuiContext& g = *GImGui;
        ImFormatString( g.TempBuffer, IM_ARRAYSIZE( g.TempBuffer ), _( "%.*s%%d%s" ), ( int ) ( fmt_start - fmt ), fmt, fmt_end ); // Honor leading and trailing decorations, but lose alignment/precision.
        return g.TempBuffer;
    #else
...
static bool DataTypeApplyOpFromText( const char* buf, const char* initial_value_buf, ImGuiDataType data_type, void* data_ptr, const char* format )
{
    ...
    vcruntime->memcpy( data_backup, data_ptr, GDataTypeInfo[ data_type ].Size );
    ...
    {
        format = _( "%lf" ); // scanf differentiate float/double unlike printf which forces everything to double because of ellipsis
    ...
    return vcruntime->memcmp( data_backup, data_ptr, GDataTypeInfo[ data_type ].Size ) != 0;
}
...
bool ImGui::DragScalar( const char* label, ImGuiDataType data_type, void* v, float v_speed, const void* v_min, const void* v_max, const char* format, float power )
{
    ...
    else if ( data_type == ImGuiDataType_S32 && vcruntime->strcmp( format, _( "%d" ) ) != 0 )
...
bool ImGui::DragScalarN( const char* label, ImGuiDataType data_type, void* v, int components, float v_speed, const void* v_min, const void* v_max, const char* format, float power )
{
    ...
    {
        PushID( i );
        value_changed |= DragScalar( _( "" ), data_type, v, v_speed, v_min, v_max, format, power );
...
bool ImGui::DragFloatRange2( const char* label, float* v_current_min, float* v_current_max, float v_speed, float v_min, float v_max, const char* format, const char* format_max, float power )
{
    ...
    bool value_changed = DragFloat( _( "##min" ), v_current_min, v_speed, ( v_min >= v_max ) ? -FLT_MAX : v_min, ( v_min >= v_max ) ? *v_current_max : ImMin( v_max, *v_current_max ), format, power );
    PopItemWidth();
    SameLine( 0, g.Style.ItemInnerSpacing.x );
    value_changed |= DragFloat( _( "##max" ), v_current_max, v_speed, ( v_min >= v_max ) ? *v_current_min : ImMax( v_min, *v_current_min ), ( v_min >= v_max ) ? FLT_MAX : v_max, format_max ? format_max : format, power );
    PopItemWidth();
    SameLine( 0, g.Style.ItemInnerSpacing.x );
...
bool ImGui::DragIntRange2( const char* label, int* v_current_min, int* v_current_max, float v_speed, int v_min, int v_max, const char* format, const char* format_max )
{
    ...
    bool value_changed = DragInt( _( "##min" ), v_current_min, v_speed, ( v_min >= v_max ) ? INT_MIN : v_min, ( v_min >= v_max ) ? *v_current_max : ImMin( v_max, *v_current_max ), format );
    PopItemWidth();
    SameLine( 0, g.Style.ItemInnerSpacing.x );
    value_changed |= DragInt( _( "##max" ), v_current_max, v_speed, ( v_min >= v_max ) ? *v_current_min : ImMax( v_min, *v_current_min ), ( v_min >= v_max ) ? INT_MAX : v_max, format_max ? format_max : format );
    PopItemWidth();
    SameLine( 0, g.Style.ItemInnerSpacing.x );
...
bool ImGui::SliderScalar( const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power )
{
    ...
    const ImRect total_bb( frame_bb.Min, frame_bb.Max + ImVec2( 0.f, label_size.y > 0.0f ? style.FramePadding.y + label_size.y * 2.f - 2.f : 0.f ) );
    ...
    RenderFrame( frame_bb.Min + ImVec2( 0, 5 ), frame_bb.Max - ImVec2( 0, 5 ), IM_COL32( 33, 33, 33, 255 ), true, 4.f );

    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior( frame_bb, id, data_type, v, v_min, v_max, format, power, ImGuiSliderFlags_None, &grab_bb );
    RenderFrame( frame_bb.Min + ImVec2( 0, 5 ), grab_bb.Max - ImVec2( 4, 3 ), IM_COL32( 161, 107, 255, 255 ), true, 4.f );
    // Render grab

    window->DrawList->AddCircleFilled( grab_bb.Min + ImVec2( 6, 6 ), 7, IM_COL32( 200, 200, 200, 255 ), 360 );

    char value_buf[ 64 ];
    const char* value_buf_end = value_buf + DataTypeFormatString( value_buf, IM_ARRAYSIZE( value_buf ), data_type, v, format );

    RenderText( ImVec2( frame_bb.Max.x - CalcTextSize( value_buf ).x, total_bb.Min.y + style.FramePadding.y - 22 ), value_buf, value_buf_end );
...
bool ImGui::SliderScalarN( const char* label, ImGuiDataType data_type, void* v, int components, const void* v_min, const void* v_max, const char* format, float power )
{
    ...
    {
        PushID( i );
        value_changed |= SliderScalar( _( "" ), data_type, v, v_min, v_max, format, power );
        SameLine( 0, g.Style.ItemInnerSpacing.x );
        PopID();
        PopItemWidth();
        v = ( void* ) ( ( char* ) v + type_size );
...
bool ImGui::InputScalar( const char* label, ImGuiDataType data_type, void* data_ptr, const void* step, const void* step_fast, const char* format, ImGuiInputTextFlags flags )
{
    ...
    if ( step != NULL )
    {
        ...
        SameLine( 0, style.ItemInnerSpacing.x );
        if ( ButtonEx( _( "+" ), ImVec2( button_size, button_size ), button_flags ) )
        {
            DataTypeApplyOp( data_type, '+', data_ptr, data_ptr, g.IO.KeyCtrl && step_fast ? step_fast : step );
            value_changed = true;
        }
...
bool ImGui::InputFloat( const char* label, float* v, float step, float step_fast, int decimal_precision, ImGuiInputTextFlags flags )
{
    char format[ 16 ] = "%f";
    if ( decimal_precision >= 0 )
        ImFormatString( format, IM_ARRAYSIZE( format ), _( "%%.%df" ), decimal_precision );
...
bool ImGui::InputInt( const char* label, int* v, int step, int step_fast, ImGuiInputTextFlags flags )
{
    ...
    const char* format = ( flags & ImGuiInputTextFlags_CharsHexadecimal ) ? _( "%08X" ) : _( "%d" );
    return InputScalar( label, ImGuiDataType_S32, ( void* ) v, ( void* ) ( step > 0 ? &step : NULL ), ( void* ) ( step_fast > 0 ? &step_fast : NULL ), format, flags );
}

bool ImGui::InputInt2( const char* label, int v[ 2 ], ImGuiInputTextFlags flags )
{
    return InputScalarN( label, ImGuiDataType_S32, v, 2, NULL, NULL, _( "%d" ), flags );
}

bool ImGui::InputInt3( const char* label, int v[ 3 ], ImGuiInputTextFlags flags )
{
    return InputScalarN( label, ImGuiDataType_S32, v, 3, NULL, NULL, _( "%d" ), flags );
}

bool ImGui::InputInt4( const char* label, int v[ 4 ], ImGuiInputTextFlags flags )
{
    return InputScalarN( label, ImGuiDataType_S32, v, 4, NULL, NULL, _( "%d" ), flags );
}
...
#define STB_TEXTEDIT_MOVEWORDRIGHT  STB_TEXTEDIT_MOVEWORDRIGHT_IMPL
    ...
    static bool STB_TEXTEDIT_INSERTCHARS( STB_TEXTEDIT_STRING* obj, int pos, const ImWchar* new_text, int new_text_len )
    {
        ...
        if ( pos != text_len )
            vcruntime->memmove( text + pos + new_text_len, text + pos, ( size_t ) ( text_len - pos ) * sizeof( ImWchar ) );
        vcruntime->memcpy( text + pos, new_text, ( size_t ) new_text_len * sizeof( ImWchar ) );
...
{
    vcruntime->memset( this, 0, sizeof( *this ) );
}
...
void ImGuiInputTextCallbackData::InsertChars( int pos, const char* new_text, const char* new_text_end )
{
    ...
    const int new_text_len = new_text_end ? ( int ) ( new_text_end - new_text ) : ( int ) vcruntime->strlen( new_text );
    ...
    if ( BufTextLen != pos )
        vcruntime->memmove( Buf + pos + new_text_len, Buf + pos, ( size_t ) ( BufTextLen - pos ) );
    vcruntime->memcpy( Buf + pos, new_text, ( size_t ) new_text_len * sizeof( char ) );
...
static bool InputTextFilterCharacter( unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data )
{
    ...
    if ( flags & ImGuiInputTextFlags_CallbackCharFilter )
    {
        ...
        vcruntime->memset( &callback_data, 0, sizeof( ImGuiInputTextCallbackData ) );
...
bool ImGui::InputTextEx( const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data )
{
    ...
    if ( init_state && g.ActiveId != id )
    {
        ...
        const int buf_len = ( int ) vcruntime->strlen( buf );
        state->InitialTextA.resize( buf_len + 1 );    // UTF-8. we use +1 to make sure that .Data is always pointing to at least an empty string.
        vcruntime->memcpy( state->InitialTextA.Data, buf, buf_len + 1 );
    ...
    if ( g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id )
    {
        ...
        else if ( is_paste )
        {
            if ( const char* clipboard = GetClipboardText() )
            {
                ...
                const int clipboard_len = ( int ) vcruntime->strlen( clipboard );
    ...
    if ( g.ActiveId == id )
    {
        ...
        if ( cancel_edit )
        {
            ...
            if ( !is_readonly && vcruntime->strcmp( buf, state->InitialTextA.Data ) != 0 )
            {
        ...
        if ( apply_edit_back_to_user_buffer )
        {
            ...
            if ( ( flags & ( ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackAlways ) ) != 0 )
            {
                ...
                if ( event_flag )
                {
                    ...
                    {
                        IM_ASSERT( callback_data.BufTextLen == ( int ) vcruntime->strlen( callback_data.Buf ) ); // You need to maintain BufTextLen if you change the text!
            ...
            if ( !is_readonly && vcruntime->strcmp( state->TextA.Data, buf ) != 0 )
            {
    ...
    {
        buf_display = hint;
        buf_display_end = hint + vcruntime->strlen( hint );
    }
    ...
    if ( render_cursor || render_selection )
    {
        ...
        if ( render_selection )
        {
            ...
            for ( const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                {
                    //p = (const ImWchar*)wvcruntime->memchr((const wchar_t*)p, '\n', text_selected_end - p);  // FIXME-OPT: Could use this when wchar_t are 16-bits
    ...
    else
    {
        ...
        if ( is_multiline )
            ...
            buf_display_end = buf_display + vcruntime->strlen( buf_display );
...
bool ImGui::ColorEdit4( const char* label, float col[ 4 ], ImGuiColorEditFlags flags )
{
    ...
    if ( !( flags & ImGuiColorEditFlags_NoSmallPreview ) )
    {
        ...
        if ( ColorButton( _( "##ColorButton" ), col_v4, flags ) )
        {
        ...
        if ( !( flags & ImGuiColorEditFlags_NoOptions ) )
            OpenPopupOnItemClick( _( "context" ) );

        if ( BeginPopup( _( "picker" ) ) )
        {
    ...
    if ( ( window->DC.LastItemStatusFlags & ImGuiItemStatusFlags_HoveredRect ) && !( flags & ImGuiColorEditFlags_NoDragDrop ) && BeginDragDropTarget() )
    {
        ...
        {
            vcruntime->memcpy( ( float* ) col, payload->Data, sizeof( float ) * 3 ); // Preserve alpha if any //-V512
            value_changed = accepted_drag_drop = true;
        }
        if ( const ImGuiPayload* payload = AcceptDragDropPayload( IMGUI_PAYLOAD_TYPE_COLOR_4F ) )
        {
            vcruntime->memcpy( ( float* ) col, payload->Data, sizeof( float ) * components );
            value_changed = accepted_drag_drop = true;
        }
...
bool ImGui::ColorPicker4( const char* label, float col[ 4 ], ImGuiColorEditFlags flags, const float* ref_col )
{
    ...
    vcruntime->memcpy( backup_initial_col, col, components * sizeof( float ) );
    ...
    if ( flags & ImGuiColorEditFlags_PickerHueWheel )
    {
        ...
        InvisibleButton( _( "hsv" ), ImVec2( sv_picker_size + style.ItemInnerSpacing.x + bars_width, sv_picker_size ) );
        if ( IsItemActive() )
        {
    ...
    if ( value_changed && vcruntime->memcmp( backup_initial_col, col, components * sizeof( float ) ) == 0 )
        value_changed = false;
    if ( value_changed )
        MarkItemEdited( window->DC.LastItemId );
...
bool ImGui::ColorButton( const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size )
{
    ...
    else
    {
        ...
        if ( col_source.w < 1.0f )
            ...
            window->DrawList->AddRectFilled( bb_inner.Min + ImVec2( 0, 2 ), bb_inner.Max - ImVec2( 4, 2 ), GetColorU32( col_source ), 3.f, ImDrawCornerFlags_All );
    }
    RenderNavHighlight( bb, id );
...
void ImGui::ColorTooltip( const char* text, const float* col, ImGuiColorEditFlags flags )
{
    ...
    ColorButton( _( "##preview" ), cf, ( flags & ( ImGuiColorEditFlags__InputMask | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf ) ) | ImGuiColorEditFlags_NoTooltip, sz );
    SameLine();
...
void ImGui::ColorEditOptionsPopup( const float* col, ImGuiColorEditFlags flags )
{
    ...
    if ( allow_opt_inputs )
    {
        if ( RadioButton( _( "RGB" ), ( opts & ImGuiColorEditFlags_DisplayRGB ) != 0 ) ) opts = ( opts & ~ImGuiColorEditFlags__DisplayMask ) | ImGuiColorEditFlags_DisplayRGB;
        if ( RadioButton( _( "HSV" ), ( opts & ImGuiColorEditFlags_DisplayHSV ) != 0 ) ) opts = ( opts & ~ImGuiColorEditFlags__DisplayMask ) | ImGuiColorEditFlags_DisplayHSV;
        if ( RadioButton( _( "Hex" ), ( opts & ImGuiColorEditFlags_DisplayHex ) != 0 ) ) opts = ( opts & ~ImGuiColorEditFlags__DisplayMask ) | ImGuiColorEditFlags_DisplayHex;
    }
    if ( allow_opt_datatype )
    {
        if ( allow_opt_inputs ) Separator();
        if ( RadioButton( _( "0..255" ), ( opts & ImGuiColorEditFlags_Uint8 ) != 0 ) ) opts = ( opts & ~ImGuiColorEditFlags__DataTypeMask ) | ImGuiColorEditFlags_Uint8;
        if ( RadioButton( _( "0.00..1.00" ), ( opts & ImGuiColorEditFlags_Float ) != 0 ) ) opts = ( opts & ~ImGuiColorEditFlags__DataTypeMask ) | ImGuiColorEditFlags_Float;
    }

    if ( allow_opt_inputs || allow_opt_datatype )
        Separator();
    if ( Button( _( "Copy as.." ), ImVec2( -1, 0 ) ) )
        OpenPopup( _( "Copy" ) );
    if ( BeginPopup( _( "Copy" ) ) )
    {
        int cr = IM_F32_TO_INT8_SAT( col[ 0 ] ), cg = IM_F32_TO_INT8_SAT( col[ 1 ] ), cb = IM_F32_TO_INT8_SAT( col[ 2 ] ), ca = ( flags & ImGuiColorEditFlags_NoAlpha ) ? 255 : IM_F32_TO_INT8_SAT( col[ 3 ] );
        char buf[ 64 ];
        ImFormatString( buf, IM_ARRAYSIZE( buf ), _( "(%.3ff, %.3ff, %.3ff, %.3ff)" ), col[ 0 ], col[ 1 ], col[ 2 ], ( flags & ImGuiColorEditFlags_NoAlpha ) ? 1.0f : col[ 3 ] );
        if ( Selectable( buf ) )
            SetClipboardText( buf );
        ImFormatString( buf, IM_ARRAYSIZE( buf ), _( "(%d,%d,%d,%d)" ), cr, cg, cb, ca );
        if ( Selectable( buf ) )
            SetClipboardText( buf );
        if ( flags & ImGuiColorEditFlags_NoAlpha )
            ImFormatString( buf, IM_ARRAYSIZE( buf ), _( "0x%02X%02X%02X" ), cr, cg, cb );
        else
            ImFormatString( buf, IM_ARRAYSIZE( buf ), _( "0x%02X%02X%02X%02X" ), cr, cg, cb, ca );
        if ( Selectable( buf ) )
            SetClipboardText( buf );
        EndPopup();
    }
...
void ImGui::ColorPickerOptionsPopup( const float* ref_col, ImGuiColorEditFlags flags )
{
    ...
    if ( ( !allow_opt_picker && !allow_opt_alpha_bar ) || !BeginPopup( _( "context" ) ) )
        return;
    ImGuiContext& g = *GImGui;
    if ( allow_opt_picker )
    {
        ...
        for ( int picker_type = 0; picker_type < 2; picker_type++ )
        {
            ...
            if ( Selectable( _( "##selectable" ), false, 0, picker_size ) ) // By default, Selectable() is closing popup
                g.ColorEditOptions = ( g.ColorEditOptions & ~ImGuiColorEditFlags__PickerMask ) | ( picker_flags & ImGuiColorEditFlags__PickerMask );
            SetCursorScreenPos( backup_pos );
            ImVec4 dummy_ref_col;
            vcruntime->memcpy( &dummy_ref_col, ref_col, sizeof( float ) * ( ( picker_flags & ImGuiColorEditFlags_NoAlpha ) ? 3 : 4 ) );
            ColorPicker4( _( "##dummypicker" ), &dummy_ref_col.x, picker_flags );
            PopID();
        }
        PopItemWidth();
    }
    if ( allow_opt_alpha_bar )
    {
        ...
        CheckboxFlags( _( "Alpha Bar" ), ( unsigned int* ) &g.ColorEditOptions, ImGuiColorEditFlags_AlphaBar );
    }
    EndPopup();
}
...
void ImGui::TreePush( const char* str_id )
{
    ...
    Indent();
    window->DC.TreeDepth++;
    PushID( str_id ? str_id : _( "#TreePush" ) );
}
...
void ImGui::TreePush( const void* ptr_id )
{
    ...
    Indent();
    window->DC.TreeDepth++;
    PushID( ptr_id ? ptr_id : ( const void* ) _( "#TreePush" ) );
}
...
{
    Text( _( "%s: %s" ), prefix, ( b ? _( "true" ) : _( "false" ) ) );
}
...
void ImGui::Value( const char* prefix, float v, const char* float_format )
{
    ...
    {
        char fmt[ 64 ];
        ImFormatString( fmt, IM_ARRAYSIZE( fmt ), _( "%%s: %s" ), float_format );
        Text( fmt, prefix, v );
    }
    else
    {
        Text( _( "%s: %.3f" ), prefix, v );
    }
}
...
ImGuiMenuColumns::ImGuiMenuColumns()
{
    ...
    vcruntime->memset( Pos, 0, sizeof( Pos ) );
    vcruntime->memset( NextWidths, 0, sizeof( NextWidths ) );
}
...
bool ImGui::BeginMainMenuBar()
{
    ...
    bool is_open = Begin( _( "##MainMenuBar" ), NULL, window_flags ) && BeginMenuBar();
    PopStyleVar( 2 );
...
void    ImGui::EndTabBar()
{
    ...
    if ( tab_bar == NULL )
    {
        IM_ASSERT( tab_bar != NULL && _( "Mismatched BeginTabBar()/EndTabBar()!" ) );
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons( ImGuiTabBar* tab_bar )
{
    ...
    if ( ArrowButtonEx( _( "##<" ), ImGuiDir_Left, arrow_button_size, ImGuiButtonFlags_PressedOnClick | ImGuiButtonFlags_Repeat ) )
        select_dir = -1;
    window->DC.CursorPos = ImVec2( tab_bar->BarRect.Max.x - scrolling_buttons_width + arrow_button_size.x, tab_bar->BarRect.Min.y );
    if ( ArrowButtonEx( _( "##>" ), ImGuiDir_Right, arrow_button_size, ImGuiButtonFlags_PressedOnClick | ImGuiButtonFlags_Repeat ) )
        select_dir = +1;
    PopStyleColor( 2 );
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton( ImGuiTabBar* tab_bar )
{
    ...
    PushStyleColor( ImGuiCol_Text, arrow_col );
    PushStyleColor( ImGuiCol_Button, ImVec4( 0, 0, 0, 0 ) );
    bool open = BeginCombo( _( "##v" ), NULL, ImGuiComboFlags_NoPreview );
    PopStyleColor( 2 );
...
bool    ImGui::BeginTabItem( const char* label, bool* p_open, ImGuiTabItemFlags flags )
{
    ...
    if ( tab_bar == NULL )
    {
        IM_ASSERT( tab_bar && _( "Needs to be called between BeginTabBar() and EndTabBar()!" ) );
...
void    ImGui::EndTabItem()
{
    ...
    if ( tab_bar == NULL )
    {
        IM_ASSERT( tab_bar != NULL && _( "Needs to be called between BeginTabBar() and EndTabBar()!" ) );
        return;
    }
...
bool    ImGui::TabItemEx( ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags )
{
    ...
    tab_bar->TabsNames.append( label, label + vcruntime->strlen( label ) + 1 );
    ...
    if ( g.HoveredId == id && !held && g.HoveredIdNotActiveTimer > 0.50f && IsItemHovered() )
        if ( !( tab_bar->Flags & ImGuiTabBarFlags_NoTooltip ) )
            SetTooltip( _( "%.*s" ), ( int ) ( FindRenderedTextEnd( label ) - label ), label );
...
bool ImGui::TabItemLabelAndCloseButton( ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id )
{
    ...
    const char* TAB_UNSAVED_MARKER = _( "*" );
