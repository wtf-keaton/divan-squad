...
static void* MallocWrapper( size_t size, void* user_data )
{
    ...
    return engine::functions::malloc( size );
}
static void FreeWrapper( void* ptr, void* user_data )
{
    IM_UNUSED( user_data );

    engine::functions::free( ptr );
}

void* operator new( size_t n )
{
    return MallocWrapper( n, 0 );
}
...
ImGuiStyle::ImGuiStyle()
{
    ...
    WindowPadding = ImVec2( 0, 8 );      // Padding within a window
...
ImGuiIO::ImGuiIO()
{
    ...
    // Input (NB: we already have vcruntime->memset zero the entire structure!)
...
{
    int d;
    while ( ( d = vcruntime->toupper( *str2 ) - vcruntime->toupper( *str1 ) ) == 0 && *str1 ) { str1++; str2++; }
    return d;
}
...
{
    int d = 0;
    while ( count > 0 && ( d = vcruntime->toupper( *str2 ) - vcruntime->toupper( *str1 ) ) == 0 && *str1 ) { str1++; str2++; count--; }
    return d;
}
...
void ImStrncpy( char* dst, const char* src, size_t count )
{
    if ( count < 1 )
        return;
    if ( count > 1 )
        vcruntime->strncpy( dst, src, count - 1 );
    dst[ count - 1 ] = 0;
}

char* ImStrdup( const char* str )
{
    size_t len = vcruntime->strlen( str );
    void* buf = ImGui::MemAlloc( len + 1 );
    return ( char* ) vcruntime->memcpy( buf, ( const void* ) str, len + 1 );
}

char* ImStrdupcpy( char* dst, size_t* p_dst_size, const char* src )
{
    size_t dst_buf_size = p_dst_size ? *p_dst_size : vcruntime->strlen( dst ) + 1;
    size_t src_size = vcruntime->strlen( src ) + 1;
    ...
    return ( char* ) vcruntime->memcpy( dst, ( const void* ) src, src_size );
}

const char* ImStrchrRange( const char* str, const char* str_end, char c )
{
    const char* p = ( const char* ) vcruntime->memchr( str, ( int ) c, str_end - str );
    return p;
}
...
{
    const char* p = ( const char* ) vcruntime->memchr( str, '\n', str_end - str );
    return p ? p : str_end;
}
...
{
    if ( !needle_end )
        needle_end = needle + vcruntime->strlen( needle );

    const char un0 = ( char ) vcruntime->toupper( *needle );
    while ( ( !haystack_end && *haystack ) || ( haystack_end && haystack < haystack_end ) )
    {
        if ( vcruntime->toupper( *haystack ) == un0 )
        {
            const char* b = needle + 1;
            for ( const char* a = haystack + 1; b < needle_end; a++, b++ )
                if ( vcruntime->toupper( *a ) != vcruntime->toupper( *b ) )
                    break;
            if ( b == needle_end )
                return haystack;
        }
...
void ImStrTrimBlanks( char* buf )
{
    ...
    if ( p_start != buf )                     // Copy memory if we had leading blanks
        vcruntime->memmove( buf, p_start, p - p_start );
...
{
    Filters.resize( 0 );
    TextRange input_range( InputBuf, InputBuf + vcruntime->strlen( InputBuf ) );
...
{
    int len = str_end ? ( int ) ( str_end - str ) : ( int ) vcruntime->strlen( str );
    ...
    vcruntime->memcpy( &Buf[ write_off - 1 ], str, ( size_t ) len );
    ...
    : DrawListInst( &context->DrawListSharedData )
{
    ...
    NameBufLen = ( int ) vcruntime->strlen( name ) + 1;
    MoveId = GetID( _( "#MOVE" ) );
    ChildId = 0;
...
{
    bool error = false;
    if ( vcruntime->strcmp( version, IMGUI_VERSION ) != 0 ) { error = true; IM_ASSERT( vcruntime->strcmp( version, IMGUI_VERSION ) == 0 && _( "Mismatched version string!") ); }
    if ( sz_io != sizeof( ImGuiIO ) )       { error = true; IM_ASSERT( sz_io == sizeof( ImGuiIO ) && _( "Mismatched struct layout!" ) ); }
    if ( sz_style != sizeof( ImGuiStyle ) )    { error = true; IM_ASSERT( sz_style == sizeof( ImGuiStyle ) && _( "Mismatched struct layout!" ) ); }
    if ( sz_vec2 != sizeof( ImVec2 ) )        { error = true; IM_ASSERT( sz_vec2 == sizeof( ImVec2 ) && _( "Mismatched struct layout!" ) ); }
    if ( sz_vec4 != sizeof( ImVec4 ) )        { error = true; IM_ASSERT( sz_vec4 == sizeof( ImVec4 ) && _( "Mismatched struct layout!" ) ); }
    if ( sz_vert != sizeof( ImDrawVert ) )    { error = true; IM_ASSERT( sz_vert == sizeof( ImDrawVert ) && _( "Mismatched struct layout!" ) ); }
    return !error;
}
...
ImGuiIO& ImGui::GetIO()
{
    IM_ASSERT( GImGui != NULL && _( "No current context. Did you call ImGui::CreateContext() or ImGui::SetCurrentContext()?" ) );
    return GImGui->IO;
}
...
void ImGui::NewFrame()
{
    ...
    IM_ASSERT( ( g.IO.DeltaTime > 0.0f || g.FrameCount == 0 ) && _( "Need a positive DeltaTime!" ) );
    IM_ASSERT( ( g.FrameCount == 0 || g.FrameCountEnded == g.FrameCount ) && _( "Forgot to call Render() or EndFrame() at the end of the previous frame?" ) );
    IM_ASSERT( g.IO.DisplaySize.x >= 0.0f && g.IO.DisplaySize.y >= 0.0f && _( "Invalid DisplaySize value!" ) );
    IM_ASSERT( g.IO.Fonts->Fonts.Size > 0 && _( "Font Atlas not built. Did you call io.Fonts->GetTexDataAsRGBA32() / GetTexDataAsAlpha8() ?" ) );
    IM_ASSERT( g.IO.Fonts->Fonts[ 0 ]->IsLoaded() && _( "Font Atlas not built. Did you call io.Fonts->GetTexDataAsRGBA32() / GetTexDataAsAlpha8() ?" ) );
    IM_ASSERT( g.Style.CurveTessellationTol > 0.0f && _( "Invalid style setting!"));
    IM_ASSERT( g.Style.Alpha >= 0.0f && g.Style.Alpha <= 1.0f && _( "Invalid style setting. Alpha cannot be negative (allows us to avoid a few clamps in color computations)!" ) );
    IM_ASSERT( g.Style.WindowMinSize.x >= 1.0f && g.Style.WindowMinSize.y >= 1.0f && _( "Invalid style setting." ) );
    for ( int n = 0; n < ImGuiKey_COUNT; n++ )
        IM_ASSERT( g.IO.KeyMap[ n ] >= -1 && g.IO.KeyMap[ n ] < IM_ARRAYSIZE( g.IO.KeysDown ) && _( "io.KeyMap[] contains an out of bound value (need to be 0..512, or -1 for unmapped key)" ) );

    // Perform simple check: required key mapping (we intentionally do NOT check all keys to not pressure user into setting up everything, but Space is required and was only recently added in 1.60 WIP)
    if ( g.IO.ConfigFlags & ImGuiConfigFlags_NavEnableKeyboard )
        IM_ASSERT( g.IO.KeyMap[ ImGuiKey_Space ] != -1 && _( "ImGuiKey_Space is not mapped, required for keyboard navigation." ) );
    ...
    vcruntime->memcpy( g.IO.KeysDownDurationPrev, g.IO.KeysDownDuration, sizeof( g.IO.KeysDownDuration ) );
...
void ImGui::Initialize( ImGuiContext* context )
{
    ...
    ini_handler.TypeHash = ImHashStr( _( "Window" ), 0 );
...
static void AddDrawListToDrawData( ImVector<ImDrawList*>* out_list, ImDrawList* draw_list )
{
    ...
    if ( sizeof( ImDrawIdx ) == 2 )
        IM_ASSERT( draw_list->_VtxCurrentIdx < ( 1 << 16 ) && _( "Too many vertices in ImDrawList using 16-bit indices. Read comment above" ) );
...
void ImDrawDataBuilder::FlattenIntoSingleLayer()
{
    ...
    for ( int layer_n = 1; layer_n < IM_ARRAYSIZE( Layers ); layer_n++ )
    {
        ...
        if ( layer.empty() )
            continue;
        vcruntime->memcpy( &Layers[ 0 ][ n ], &layer[ 0 ], layer.Size * sizeof( ImDrawList* ) );
        n += layer.Size;
        layer.resize( 0 );
    }
}
...
void ImGui::EndFrame()
{
    ...
    if ( g.FrameCountEnded == g.FrameCount )          // Don't process EndFrame() multiple times.
        return;
    IM_ASSERT( g.FrameScopeActive && _( "Forgot to call ImGui::NewFrame()?" ) );
    ...
    if ( g.CurrentWindowStack.Size != 1 )
    {
        if ( g.CurrentWindowStack.Size > 1 )
        {
            IM_ASSERT( g.CurrentWindowStack.Size == 1 && _( "Mismatched Begin/BeginChild vs End/EndChild calls: did you forget to call End/EndChild?" ) );
            while ( g.CurrentWindowStack.Size > 1 ) // FIXME-ERRORHANDLING
                End();
        }
        else
        {
            IM_ASSERT( g.CurrentWindowStack.Size == 1 && _( "Mismatched Begin/BeginChild vs End/EndChild calls: did you call End/EndChild too much?" ) );
        }
    }
    ...
    vcruntime->memset( g.IO.NavInputs, 0, sizeof( g.IO.NavInputs ) );
}
...
static bool ImGui::BeginChildEx( const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags )
{
    ...
    char title[ 256 ];
    if ( name )
        ImFormatString( title, IM_ARRAYSIZE( title ), _( "%s/%s_%08X" ), parent_window->Name, name, id );
    else
        ImFormatString( title, IM_ARRAYSIZE( title ), _( "%s/%08X" ), parent_window->Name, id );
...
static void CheckStacksSize( ImGuiWindow* window, bool write )
{
    ...
    { int current = window->IDStack.Size;       if ( write ) *p_backup = ( short ) current; else IM_ASSERT( *p_backup == current && _( "PushID/PopID or TreeNode/TreePop Mismatch!" ) );   p_backup++; }    // Too few or too many PopID()/TreePop()
    { int current = window->DC.GroupStack.Size; if ( write ) *p_backup = ( short ) current; else IM_ASSERT( *p_backup == current && _( "BeginGroup/EndGroup Mismatch!" ) );                p_backup++; }    // Too few or too many EndGroup()
    { int current = g.BeginPopupStack.Size;     if ( write ) *p_backup = ( short ) current; else IM_ASSERT( *p_backup == current && _( "BeginMenu/EndMenu or BeginPopup/EndPopup Mismatch" ) ); p_backup++; }// Too few or too many EndMenu()/EndPopup()
    // For color, style and font stacks there is an incentive to use Push/Begin/Pop/.../End patterns, so we relax our checks a little to allow them.
    { int current = g.ColorModifiers.Size;      if ( write ) *p_backup = ( short ) current; else IM_ASSERT( *p_backup >= current && _( "PushStyleColor/PopStyleColor Mismatch!" ) );       p_backup++; }    // Too few or too many PopStyleColor()
    { int current = g.StyleModifiers.Size;      if ( write ) *p_backup = ( short ) current; else IM_ASSERT( *p_backup >= current && _( "PushStyleVar/PopStyleVar Mismatch!" ) );           p_backup++; }    // Too few or too many PopStyleVar()
    { int current = g.FontStack.Size;           if ( write ) *p_backup = ( short ) current; else IM_ASSERT( *p_backup >= current && _( "PushFont/PopFont Mismatch!" ) );                   p_backup++; }    // Too few or too many PopFont()
...
bool ImGui::Begin( const char* name, bool* p_open, ImGuiWindowFlags flags )
{
    ...
    if ( first_begin_of_the_frame )
    {
        ...
        if ( window_title_visible_elsewhere && !window_just_created && vcruntime->strcmp( name, window->Name ) != 0 )
        {
        ...
        if ( !( flags & ImGuiWindowFlags_NoTitleBar ) )
        {
            ...
            if ( !( flags & ImGuiWindowFlags_NoCollapse ) )
                if ( CollapseButton( window->GetID( _( "#COLLAPSE" ) ), window->Pos ) )
            ...
            if ( p_open != NULL )
            {
                ...
                if ( CloseButton( window->GetID( _( "#CLOSE" ) ), window->Rect().GetTR() + ImVec2(-pad - rad, pad + rad), rad + 1) )
                    *p_open = false;
            }
...
void ImGui::End()
{
    ...
    {
        IM_ASSERT( g.CurrentWindowStack.Size > 1 && _( "Calling End() too many times!" ) );
...
void ImGui::BringWindowToFocusFront( ImGuiWindow* window )
{
    ...
    for ( int i = g.WindowsFocusOrder.Size - 2; i >= 0; i-- ) // We can ignore the front most window
        ...
        {
            vcruntime->memmove( &g.WindowsFocusOrder[ i ], &g.WindowsFocusOrder[ i + 1 ], ( size_t ) ( g.WindowsFocusOrder.Size - i - 1 ) * sizeof( ImGuiWindow* ) );
...
void ImGui::BringWindowToDisplayFront( ImGuiWindow* window )
{
    ...
    for ( int i = g.Windows.Size - 2; i >= 0; i-- ) // We can ignore the front most window
        ...
        {
            vcruntime->memmove( &g.Windows[ i ], &g.Windows[ i + 1 ], ( size_t ) ( g.Windows.Size - i - 1 ) * sizeof( ImGuiWindow* ) );
...
void ImGui::BringWindowToDisplayBack( ImGuiWindow* window )
{
    ...
    for ( int i = 0; i < g.Windows.Size; i++ )
        if ( g.Windows[ i ] == window )
        {
            vcruntime->memmove( &g.Windows[ 1 ], &g.Windows[ 0 ], ( size_t ) i * sizeof( ImGuiWindow* ) );
            g.Windows[ 0 ] = window;
            break;
        }
}
...
void ImGui::PopStyleVar( int count )
{
    ...
    while ( count > 0 )
    {
        // We avoid a generic vcruntime->memcpy(data, &backup.Backup.., GDataTypeSize[info->Type] * info->Count), the overhead in Debug is not worth it.
...
bool ImGui::BeginPopupEx( ImGuiID id, ImGuiWindowFlags extra_flags )
{
    ...
    if ( extra_flags & ImGuiWindowFlags_ChildMenu )
        ImFormatString( name, IM_ARRAYSIZE( name ), _( "##Menu_%02d" ), g.BeginPopupStack.Size ); // Recycle windows based on depth
    else
        ImFormatString( name, IM_ARRAYSIZE( name ), _( "##Popup_%08x" ), id ); // Not recycling, so we can close/open during the same frame
...
#undef NAV_MAP_KEY
    ...
    vcruntime->memcpy( g.IO.NavInputsDownDurationPrev, g.IO.NavInputsDownDuration, sizeof( g.IO.NavInputsDownDuration ) );
...
static const char* GetFallbackWindowNameForWindowingList( ImGuiWindow* window )
{
    if ( window->Flags & ImGuiWindowFlags_Popup )
        return _( "(Popup)" );
    if ( ( window->Flags & ImGuiWindowFlags_MenuBar ) && vcruntime->strcmp( window->Name, _( "##MainMenuBar" ) ) == 0 )
        return _( "(Main menu bar)" );
    return _( "(Untitled)" );
}
...
void ImGui::ClearDragDrop()
{
    ...
    vcruntime->memset( &g.DragDropPayloadBufLocal, 0, sizeof( g.DragDropPayloadBufLocal ) );
}
...
bool ImGui::BeginDragDropSource( ImGuiDragDropFlags flags )
{
    ...
    {
        window = NULL;
        source_id = ImHashStr( _( "#SourceExtern" ), 0 );
...
void ImGui::EndDragDropSource()
{
    ...
    IM_ASSERT( g.DragDropWithinSourceOrTarget && _( "Not after a BeginDragDropSource()?" ) );
...
bool ImGui::SetDragDropPayload( const char* type, const void* data, size_t data_size, ImGuiCond cond )
{
    ...
    IM_ASSERT( type != NULL );
    IM_ASSERT( vcruntime->strlen( type ) < IM_ARRAYSIZE( payload.DataType ) && _( "Payload type can be at most 32 characters long" ) );
    ...
    if ( cond == ImGuiCond_Always || payload.DataFrameCount == -1 )
    {
        ...
        if ( data_size > sizeof( g.DragDropPayloadBufLocal ) )
        {
            ...
            vcruntime->memcpy( payload.Data, data, data_size );
        }
        else if ( data_size > 0 )
        {
            // Store locally
...
void ImGui::LoadIniSettingsFromMemory( const char* ini_data, size_t ini_size )
{
    ...
    if ( ini_size == 0 )
        ini_size = vcruntime->strlen( ini_data );
    char* buf = ( char* ) ImGui::MemAlloc( ini_size + 1 );
    char* buf_end = buf + ini_size;
    vcruntime->memcpy( buf, ini_data, ini_size );
    buf[ ini_size ] = 0;
...
static void SettingsHandlerWindow_WriteAll( ImGuiContext* imgui_ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf )
{
    ...
    for ( int i = 0; i != g.SettingsWindows.Size; i++ )
    {
        ...
        if ( const char* p = vcruntime->strstr( name, _( "###" ) ) )  // Skip to the "###" marker if any. We don't skip past to match the behavior of GetID()
            name = p;
        buf->appendf( _( "[%s][%s]\n" ), handler->TypeName, name );
        buf->appendf( _( "Pos=%d,%d\n" ), ( int ) settings->Pos.x, ( int ) settings->Pos.y );
        buf->appendf( _( "Size=%d,%d\n" ), ( int ) settings->Size.x, ( int ) settings->Size.y );
        buf->appendf( _( "Collapsed=%d\n" ), settings->Collapsed );
        buf->appendf( _( "\n" ) );
    }
}
...
static void SetClipboardTextFn_DefaultImpl( void*, const char* text )
{
    ImGuiContext& g = *GImGui;
    g.PrivateClipboard.clear();
    const char* text_end = text + vcruntime->strlen( text );
    g.PrivateClipboard.resize( ( int ) ( text_end - text ) + 1 );
    vcruntime->memcpy( &g.PrivateClipboard[ 0 ], text, ( size_t ) ( text_end - text ) );
